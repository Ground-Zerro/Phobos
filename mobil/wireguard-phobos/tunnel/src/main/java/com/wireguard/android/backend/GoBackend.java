...
public final class GoBackend implements Backend {
    ...
    private void setStateInternal(final Tunnel tunnel, @Nullable final Config config, final State state)
        ...
        if (state == State.UP) {
            ...
            // Build config: start obfuscator engines (if enabled) and rewrite endpoints
            final Config goBackendConfig = ObfuscatorIntegration.bringUp(context, config, tunnel.getName(), service);
            final String goConfig = goBackendConfig.toWgUserspaceString();
    ...
    public static class VpnService extends android.net.VpnService {
        ...
        public void onDestroy() {
            ...
            try {
                if (vpnService.get(0, TimeUnit.NANOSECONDS) == this) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S)
                        vpnService = vpnService.newIncompleteFuture();
                    else
                        vpnService = new CompletableFuture<>();

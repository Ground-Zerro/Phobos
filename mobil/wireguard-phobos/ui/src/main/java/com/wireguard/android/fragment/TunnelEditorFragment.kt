...
class TunnelEditorFragment : BaseFragment(), MenuProvider {
    ...
    private fun onConfigLoaded(config: Config) {
        val configProxy = ConfigProxy(config)
        binding?.config = configProxy
        val loadedTunnel = tunnel ?: return
        val ctx = (activity ?: Application.get()).applicationContext
        lifecycleScope.launch {
            val settings = withContext(Dispatchers.IO) {
                ObfuscatorConfigStore(ctx).load(loadedTunnel.name)
            }
            if (settings.isEmpty() || binding?.config !== configProxy) return@launch
            configProxy.peers.forEach { peer ->
                val publicKey = peer.publicKey
                if (publicKey.isNotEmpty())
                    settings[publicKey]?.let { peer.applyObfuscatorSettings(it) }
            }
        }
    }

    private suspend fun writeObfuscatorSettings(tunnelName: String) {
        val peers = binding?.config?.peers ?: return
        val settings = HashMap<String, ObfuscatorPeerSettings>()
        peers.forEach { peer ->
            val publicKey = peer.publicKey
            if (publicKey.isNotEmpty())
                settings[publicKey] = peer.toObfuscatorSettings()
        }
        val ctx = (activity ?: Application.get()).applicationContext
        withContext(Dispatchers.IO) {
            runCatching { ObfuscatorConfigStore(ctx).save(tunnelName, settings) }
                .onFailure { Log.e(TAG, "Failed to save obfuscator settings", it) }
        }
    }
            ...
            activity.lifecycleScope.launch {
                when {
                    tunnel == null -> {
                        ...
                        try {
                            val createdTunnel = manager.create(binding!!.name!!, newConfig)
                            writeObfuscatorSettings(createdTunnel.name)
                            onTunnelCreated(createdTunnel, null)
                        } catch (e: Throwable) {
                    ...
                    tunnel!!.name != binding!!.name -> {
                        ...
                        try {
                            writeObfuscatorSettings(tunnel!!.name)
                        ...
                        try {
                            ...
                            val wireGuardChanged = newConfig != tunnel!!.config
                            writeObfuscatorSettings(tunnel!!.name)
                            tunnel!!.setConfigAsync(newConfig)
                            if (wasUp && !wireGuardChanged) {

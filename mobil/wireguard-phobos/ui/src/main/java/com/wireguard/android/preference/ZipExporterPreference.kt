                    ...
                    try {
                        ZipOutputStream(outputFile.outputStream).use { zip ->
                            for (i in configs.indices) {
                                ...
                                val obfuscator = ObfuscatorConfigStore(context).load(tunnels[i].name)
                                val text = ObfuscatorConfigParser.appendInstanceSections(
                                    configs[i].toWgQuickString(), configs[i], obfuscator
                                )

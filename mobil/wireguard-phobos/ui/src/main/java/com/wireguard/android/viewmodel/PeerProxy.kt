...
import com.wireguard.android.backend.obfuscator.Masking
import com.wireguard.android.backend.obfuscator.ObfuscatorPeerSettings
...
class PeerProxy : BaseObservable, Parcelable {
    ...
    @get:Bindable
    var obfuscatorEnabled: Boolean = false
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscatorEnabled)
        }

    @get:Bindable
    var obfuscationServer: String = ""
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscationServer)
        }
    ...
    var obfuscationKey: String = ""
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscationKey)
        }

    @get:Bindable
    var obfuscationObfuscateBytes: String = "0"
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscationObfuscateBytes)
        }

    @get:Bindable
    var obfuscationMaskingType: String = Masking.NONE
        set(value) {
            val normalized = Masking.normalize(value)
            field = normalized
            notifyPropertyChanged(BR.obfuscationMaskingType)
            notifyPropertyChanged(BR.maskingMedia)
            val bytes = obfuscationObfuscateBytes.toIntOrNull() ?: 0
            if (normalized == Masking.MEDIA && bytes == 0)
                obfuscationObfuscateBytes = ObfuscatorPeerSettings.MEDIA_OBFUSCATE_BYTES_DEFAULT.toString()
            else if (normalized != Masking.MEDIA && bytes == ObfuscatorPeerSettings.MEDIA_OBFUSCATE_BYTES_DEFAULT)
                obfuscationObfuscateBytes = "0"
        }

    @get:Bindable
    val isMaskingMedia: Boolean
        get() = obfuscationMaskingType == Masking.MEDIA

    @get:Bindable
    var obfuscationMaxDummy: String = ObfuscatorPeerSettings.DEFAULT_MAX_DUMMY.toString()
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscationMaxDummy)
        }

    @get:Bindable
    var obfuscationMediaPt: String = "0"
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscationMediaPt)
        }

    @get:Bindable
    var obfuscationMediaSsrc: String = "0"
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscationMediaSsrc)
        }

    @get:Bindable
    var obfuscationMediaClock: String = "0"
        set(value) {
            field = value
            notifyPropertyChanged(BR.obfuscationMediaClock)
        }
    ...
    private constructor(parcel: Parcel) {
        ...
        obfuscatorEnabled = parcel.readInt() != 0
        obfuscationServer = parcel.readString() ?: ""
        obfuscationKey = parcel.readString() ?: ""
        obfuscationMaskingType = parcel.readString() ?: Masking.NONE
        obfuscationMaxDummy = parcel.readString() ?: ObfuscatorPeerSettings.DEFAULT_MAX_DUMMY.toString()
        obfuscationObfuscateBytes = parcel.readString() ?: "0"
        obfuscationMediaPt = parcel.readString() ?: "0"
        obfuscationMediaSsrc = parcel.readString() ?: "0"
        obfuscationMediaClock = parcel.readString() ?: "0"
    }
    ...
    override fun writeToParcel(dest: Parcel, flags: Int) {
        ...
        dest.writeInt(if (obfuscatorEnabled) 1 else 0)
        dest.writeString(obfuscationServer)
        dest.writeString(obfuscationKey)
        dest.writeString(obfuscationMaskingType)
        dest.writeString(obfuscationMaxDummy)
        dest.writeString(obfuscationObfuscateBytes)
        dest.writeString(obfuscationMediaPt)
        dest.writeString(obfuscationMediaSsrc)
        dest.writeString(obfuscationMediaClock)
    }

    fun applyObfuscatorSettings(settings: ObfuscatorPeerSettings) {
        obfuscatorEnabled = settings.enabled
        obfuscationServer = settings.serverEndpoint
        obfuscationKey = settings.key
        obfuscationMaskingType = settings.maskingType
        obfuscationMaxDummy = settings.maxDummy.toString()
        obfuscationObfuscateBytes = settings.obfuscateBytes.toString()
        obfuscationMediaPt = settings.mediaPayloadType.toString()
        obfuscationMediaSsrc = settings.mediaSsrc.toString()
        obfuscationMediaClock = settings.mediaClock.toString()
    }

    fun toObfuscatorSettings(): ObfuscatorPeerSettings {
        val maskingType = Masking.normalize(obfuscationMaskingType)
        return ObfuscatorPeerSettings(
            enabled = obfuscatorEnabled,
            serverEndpoint = obfuscationServer.trim(),
            key = obfuscationKey,
            maskingType = maskingType,
            maxDummy = obfuscationMaxDummy.toIntOrNull() ?: ObfuscatorPeerSettings.DEFAULT_MAX_DUMMY,
            obfuscateBytes = obfuscationObfuscateBytes.toIntOrNull()
                ?: if (maskingType == Masking.MEDIA) ObfuscatorPeerSettings.MEDIA_OBFUSCATE_BYTES_DEFAULT else 0,
            mediaPayloadType = obfuscationMediaPt.toIntOrNull() ?: 0,
            mediaSsrc = obfuscationMediaSsrc.toLongOrNull() ?: 0,
            mediaClock = obfuscationMediaClock.toIntOrNull() ?: 0
        )
    }

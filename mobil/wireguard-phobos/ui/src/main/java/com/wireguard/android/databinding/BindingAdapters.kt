...
object BindingAdapters {
    ...
    fun interface OnMaskingModeChanged {
        fun onChanged(mode: String)
    }

    @JvmStatic
    @BindingAdapter(value = ["maskingMode", "onMaskingModeChanged"], requireAll = false)
    fun setMaskingMode(view: MaterialAutoCompleteTextView, mode: String?, listener: OnMaskingModeChanged?) {
        val context = view.context
        val labels = arrayOf(
            context.getString(R.string.obfuscation_masking_none),
            context.getString(R.string.obfuscation_masking_stun),
            context.getString(R.string.obfuscation_masking_media)
        )
        if (view.adapter == null)
            view.setAdapter(ArrayAdapter(context, android.R.layout.simple_list_item_1, labels))
        val index = Masking.ids.indexOf(Masking.normalize(mode)).coerceAtLeast(0)
        if (view.text?.toString() != labels[index])
            view.setText(labels[index], false)
        view.onItemClickListener = AdapterView.OnItemClickListener { _, _, position, _ ->
            listener?.onChanged(Masking.ids[position])
        }
    }
}

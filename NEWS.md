...
Phobos — комплексное решение для автоматизации развертывания WireGuard с обфускацией трафика. Включает серверные скрипты, клиентские установщики и Telegram-бот для управления.

## Таблица изменений

| Дата | Что сделано | Почему (причина, проблема) |
|------|-------------|----------------------------|
| **2026-01-26** | Замена Python на darkhttpd и bash | Python ненадёжен на некоторых системах, может отсутствовать или быть устаревшим. HTTP-сервер заменён на darkhttpd (собирается при установке), cleanup-db использует bash+sqlite3. Зависимость от python3 удалена. |
| **2026-01-23** | Исправлена порча бинарников при сборке пакета | Команда `sed` для конвертации CRLF применялась ко всем файлам включая бинарники wg-obfuscator, что приводило к "Bus error" на клиенте. Теперь директория `bin/` исключена из обработки. |
| 2026-01-23 | Объединены пункты меню "Пересоздать" и "Пересобрать пакет" | Логическое объединение связанных операций - пересоздание клиента всегда требует пересборки пакета. Теперь один пункт "Пересоздать клиента" выполняет обе операции последовательно. |
| 2026-01-23 | Умная генерация ссылки с проверкой изменений конфига | При создании ссылки проверяется соответствие конфигурации клиента текущим параметрам сервера (IP, порты, ключи). При обнаружении изменений предлагается пересоздать клиента с актуальными данными. |
| 2026-01-23 | Локальное определение IPv6 адреса сервера | Внешние сервисы (ifconfig.me, icanhazip.com) зависали или были недоступны. Теперь IPv6 извлекается из сетевого интерфейса через `ip addr`, исключая приватные (fc00::/fd00::) и link-local адреса. |
| **2026-01-22** | Удален fallback-код и миграционные скрипты | Проект предназначен для чистой установки на Ubuntu VPS. Удалены механизмы обратной совместимости, миграции и промежуточные решения для упрощения кодовой базы. |
| **2026-01-21** | Централизация конфигурации в `server.env` | Все настройки (ключи, IP, порты, диапазоны сетей) объединены в одном файле. Скрипты адаптированы для чтения параметров из `server.env` вместо использования разрозненных файлов или хардкода. |
| 2026-01-21 | Рефакторинг серверных скриптов | Консолидация разрозненных скриптов в модульную структуру. Удалено 15 устаревших файлов, улучшена читаемость и поддержка. |
| 2026-01-21 | Динамическая конфигурация клиентской маршрутизации | Устранение хардкода подсети `10.25.0.0/16` в клиентских скриптах. Теперь маршруты и проверки на клиенте (Linux, Keenetic, OpenWrt) автоматически адаптируются под подсеть, выданную сервером в конфигурации. |
| 2026-01-21 | Улучшено меню `phobos` | Добавлен запуск/перезапуск служб одной командой и групповые операции (стоп/старт всего стека), статус сервисов в реальном времени. |
| 2026-01-21 | Реализована смена пула адресов WireGuard с обновлением клиентов | Возможность менять внутреннюю подсеть VPN на лету. Скрипт автоматически обновляет конфиг сервера, пересчитывает IP всех существующих клиентов и обновляет их файлы конфигурации. |
| 2026-01-21 | Мгновенное применение настроек obfuscator | Убраны лишние шаги "Предпросмотр" -> "Применить". Изменения настроек (порт, ключ, параметры) применяются сразу с автоматическим перезапуском службы. |
| **2026-01-06** | Реализован комбинированный подход определения платформы | Поиск по ключевым словам в uname -a ненадежен - пользователь может изменять hostname. Теперь приоритет: специфичные файлы (/etc/openwrt_release, /opt/etc/ndm), затем init-системы (procd, systemd), и только в крайнем случае uname |
| **2025-12-25** | Добавлен Telegram-бот для управления клиентами | Упрощение управления клиентами, автоматизация мониторинга, система уровней пользователей, watchdog для очистки неактивных |
| **2025-12-12** | Добавлена поддержка роутеров ImmortalWrt | Расширение поддерживаемых платформ - ImmortalWrt является популярным форком OpenWrt с дополнительными возможностями |
| 2025-12-12 | Добавлен флаг --no-check-certificate для wget на embedded устройствах | Встроенные системы (OpenWrt, Keenetic, Netcraze, ImmortalWrt) часто имеют проблемы с проверкой SSL сертификатов из-за устаревших корневых сертификатов |
| 2025-12-12 | Добавлен режим установки для интеграции с 3x-ui панелью | Автоматическое определение 3x-ui на Linux системах, развертывание только obfuscator без WireGuard (управление через 3x-ui), автоматическая интеграция конфигурации через скрипт 3xui.py |

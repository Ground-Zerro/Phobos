...
### Зачем нужен jq на роутере?

jq - это утилита для корректного парсинга JSON. Она необходима для:
...
Устанавливается автоматически при развертывании Phobos на роутере.
...
Установите недостающие пакеты через Entware:
```bash
opkg update

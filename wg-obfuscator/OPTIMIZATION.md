...
- Бинарники можно собирать на новых системах для работы на старых без ошибки GLIBC version not found
...
|------|-------------|----------|------|-------------|
| wg-obfuscator_x86_64 | x86_64 | static | SSE2/AVX2 | -O3 -march=x86-64 -msse2 -fweb -static -static-libgcc |
...
Все бинарники статически слинкованы для работы без внешних зависимостей.
Fallback версии исключены - для каждой архитектуры используется максимально оптимизированный код.

### Ключевые улучшения:
1. **Статическая линковка для всех архитектур** - полная независимость от версии GLIBC и других библиотек
2. **Runtime detection для x86_64** - автоматический выбор AVX2 или SSE2 в зависимости от процессора
3. **Условная компиляция AVX2** - атрибут `__attribute__((target("avx2")))` без глобального флага
4. **Настоящие SIMD инструкции** - для ARM/AARCH64 и x86/x86_64
5. **Улучшенная скалярная версия** - x8 loop unrolling для MIPS
6. **Расширенная поддержка архитектур** - добавлены i686 и ARMv7
7. **Автоматическая установка зависимостей** - build-all-architectures.sh устанавливает все необходимое без запросов

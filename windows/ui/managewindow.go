...
const (
	manageWindowWindowClass = "Phobos UI - Manage Tunnels"
...
func NewManageTunnelsWindow() (*ManageTunnelsWindow, error) {
	...
	win.ChangeWindowMessageFilterEx(mtw.Handle(), win.WM_COPYDATA, win.MSGFLT_ALLOW, nil)
	...
	if systemMenu != 0 {
		win.InsertMenuItem(systemMenu, 0, true, &win.MENUITEMINFO{
			...
			DwTypeData: windows.StringToUTF16Ptr(l18n.Sprintf("&About Phobos…")),
			WID:        uint32(aboutPhobosCmd),
		})
...
func (mtw *ManageTunnelsWindow) WndProc(hwnd win.HWND, msg uint32, wParam, lParam uintptr) uintptr {
	...
	case win.WM_SYSCOMMAND:
		if wParam == aboutPhobosCmd {
			onAbout(mtw)
			return 0
		}
	case win.WM_COPYDATA:
		return mtw.onImportLinkMessage(lParam)
	case raiseMsg:

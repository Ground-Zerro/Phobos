...
type obfuscatorView struct {
	server    *labelTextLine
	masking   *labelTextLine
	login     *labelTextLine
	localPort *labelTextLine
	transfer  *labelTextLine
	lines     []widgetsLine
}
...
type ConfView struct {
	*walk.ScrollView
	...
	obfuscatorGroup *walk.GroupBox
	obfuscator      *obfuscatorView
...
func newPeerView(parent walk.Container) (*peerView, error) {
	...
	items := []labelTextLineItem{
		...
		{l18n.Sprintf("Obfuscator:"), &pv.obfuscator},
...
func newObfuscatorView(parent walk.Container) (*obfuscatorView, error) {
	ov := new(obfuscatorView)

	items := []labelTextLineItem{
		{l18n.Sprintf("Server:"), &ov.server},
		{l18n.Sprintf("Masking:"), &ov.masking},
		{l18n.Sprintf("SOCKS5 login:"), &ov.login},
		{l18n.Sprintf("Local proxy:"), &ov.localPort},
		{l18n.Sprintf("Transfer:"), &ov.transfer},
	}
	var err error
	if ov.lines, err = createLabelTextLines(items, parent, nil); err != nil {
		return nil, err
	}

	layoutInGrid(ov, parent.Layout().(*walk.GridLayout))

	return ov, nil
}

func (ov *obfuscatorView) widgetsLines() []widgetsLine {
	return ov.lines
}

func (ov *obfuscatorView) apply(o *conf.Obfuscation) {
	ov.server.show(o.Target.String())
	ov.masking.show(o.Masking.String())

	if len(o.Login) > 0 && IsAdmin {
		ov.login.show(o.Login)
	} else {
		ov.login.hide()
	}

	if o.SourceListenPort > 0 {
		ov.localPort.show(fmt.Sprintf("127.0.0.1:%d", o.SourceListenPort))
	} else {
		ov.localPort.hide()
	}

	if o.RxBytes > 0 || o.TxBytes > 0 {
		ov.transfer.show(l18n.Sprintf("%s received, %s sent", o.RxBytes.String(), o.TxBytes.String()))
	} else {
		ov.transfer.hide()
	}
}

func obfuscatorSummary(o *conf.Obfuscation) string {
	return l18n.Sprintf("%s, masking %s", o.Target.String(), o.Masking.String())
}
...
func (iv *interfaceView) apply(c *conf.Interface, socks5 bool) {
	switch {
	case socks5:
		iv.publicKey.hide()
	case IsAdmin:
...
func (pv *peerView) apply(c *conf.Peer) {
	...
	if c.Obfuscation != nil {
		pv.obfuscator.show(obfuscatorSummary(c.Obfuscation))
	} else {
		pv.obfuscator.hide()
	}
...
func (cv *ConfView) applyObfuscator(config *conf.Config) {
	if config.Obfuscation == nil {
		if cv.obfuscatorGroup != nil {
			cv.obfuscatorGroup.SetVisible(false)
		}
		return
	}
	if cv.obfuscatorGroup == nil {
		...
		group.SetTitle(l18n.Sprintf("Obfuscator"))
		view, err := newObfuscatorView(group)
		if err != nil {
			group.Dispose()
			return
		}
		cv.obfuscatorGroup, cv.obfuscator = group, view
	}
	cv.obfuscatorGroup.SetVisible(true)
	cv.obfuscator.apply(config.Obfuscation)
}
...
func (cv *ConfView) setTunnel(tunnel *manager.Tunnel, config *conf.Config, state manager.TunnelState) {
	...
	cv.interfaze.apply(&config.Interface, config.IsSocks5())
	cv.interfaze.status.update(state)
	cv.interfaze.toggleActive.update(state)
	cv.applyObfuscator(config)

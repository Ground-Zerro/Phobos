...
func (tray *Tray) setup() error {
	...
	tray.SetToolTip(l18n.Sprintf("Phobos: Deactivated"))
	...
	for _, item := range [...]struct {
		...
		{separator: true},
		{label: l18n.Sprintf("&About Phobos…"), handler: tray.onAbout, enabled: true},
			...
			if tunnelAction != nil {
				...
				case manager.TunnelStarted:
					if !wasChecked {
						...
						tray.ShowCustom(l18n.Sprintf("Phobos Activated"), l18n.Sprintf("The %s tunnel has been activated.", tunnel.Name), icon)
					}

				case manager.TunnelStopped:
					if wasChecked {
						icon, _ := loadSystemIcon("imageres", -31, 128) // TODO: this icon isn't very good...
						tray.ShowCustom(l18n.Sprintf("Phobos Deactivated"), l18n.Sprintf("The %s tunnel has been deactivated.", tunnel.Name), icon)
					}
				}
			}
		} else if !tray.mtw.Visible() {
			tray.ShowError(l18n.Sprintf("Phobos Tunnel Error"), err.Error())
		}
...
func (tray *Tray) updateGlobalState(globalState manager.TunnelState) {
	...
	tray.SetToolTip(l18n.Sprintf("Phobos: %s", textForState(globalState, true)))

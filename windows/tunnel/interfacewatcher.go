...
func (a driverAdapter) Reconfigure(config *conf.Config) error {
	if state, err := a.AdapterState(); err != nil || state != driver.AdapterStateDown {
		return err
	}
	log.Println("Reinitializing adapter configuration")
	var failures []error
	if err := a.SetConfiguration(config.ToDriverConfiguration()); err != nil {
		failures = append(failures, fmt.Errorf("%v: %w", services.ErrorDeviceSetConfig, err))
	}
	if err := a.SetAdapterState(driver.AdapterStateUp); err != nil {
		failures = append(failures, fmt.Errorf("%v: %w", services.ErrorDeviceBringUp, err))
	}
...
func watchInterface() (*interfaceWatcher, error) {
	...
	iw.interfaceChangeCallback, err = winipcfg.RegisterInterfaceChangeCallback(func(notificationType winipcfg.MibNotificationType, iface *winipcfg.MibIPInterfaceRow) {
		...
		if err := iw.adapter.Reconfigure(iw.conf); err != nil {
			log.Println(err)
		}
	})
	if err != nil {
...
func (iw *interfaceWatcher) Configure(adapter tunnelAdapter, conf *conf.Config, luid winipcfg.LUID) {
	iw.setupMutex.Lock()
	defer iw.setupMutex.Unlock()

...
// Fundamental Phobos specific WFP objects.
...
func createWfpSession() (uintptr, error) {
	sessionDisplayData, err := createWtFwpmDisplayData0("Phobos", "Phobos dynamic session")
	if err != nil {
...
func registerBaseObjects(session uintptr) (*baseObjects, error) {
	...
	//
	{
		displayData, err := createWtFwpmDisplayData0("Phobos", "Phobos provider")
		if err != nil {
			return nil, wrapErr(err)
		}
	...
	//
	{
		displayData, err := createWtFwpmDisplayData0("Phobos filters", "Permissive and blocking filters")
		if err != nil {
			return nil, wrapErr(err)
		}
...
func EnableFirewall(luid uint64, doNotRestrict bool, restrictToDNSServers []netip.Addr) error {
	...
	objectInstaller := func(session uintptr) error {
		...
		err = permitPhobosService(session, baseObjects, 15)
		if err != nil {
			return wrapErr(err)
		}

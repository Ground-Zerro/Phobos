...
startOver:
	...
	routedPrefixes := conf.RoutedPrefixes()
	routes := make(map[winipcfg.RouteData]bool, len(routedPrefixes))
	...
	for _, allowedip := range routedPrefixes {
...
func enableFirewall(conf *conf.Config, luid winipcfg.LUID) error {
	...
	if conf.IsSocks5() {
		doNotRestrict = false
	} else if len(conf.Peers) == 1 && !conf.Interface.TableOff {

...
set WIX_CANDLE_FLAGS=-nologo -dPHOBOS_VERSION="%PHOBOS_VERSION%"
...
:build
	...
	echo [+] Signing
	signtool sign %SigningProvider% /fd sha256 /tr "%TimestampServer%" /td sha256 /d "Phobos Setup" "dist\phobos-*-%PHOBOS_VERSION%.msi" || goto :error
...
:msi
	...
	echo [+] Signing %1
	signtool sign %SigningProvider% /fd sha256 /tr "%TimestampServer%" /td sha256 /d "Phobos Setup Custom Actions" "%~1\customactions.dll" || exit /b 1
:skipsign
	"%WIX%bin\candle" %WIX_CANDLE_FLAGS% -dPHOBOS_PLATFORM="%~1" -out "%~1\phobos.wixobj" -arch %3 phobos.wxs || exit /b %errorlevel%
	echo [+] Linking %1
	"%WIX%bin\light" %WIX_LIGHT_FLAGS% -out "dist\phobos-%~1-%PHOBOS_VERSION%.msi" "%~1\phobos.wixobj" || exit /b %errorlevel%
	goto :eof

...
Phobos consists of four components: a kernel driver, and three separate interacting userspace parts.
...
The manager service is a userspace service running as Local System, responsible for starting and stopping tunnel services, and ensuring a UI program with certain handles is available to Administrators. It exposes:
  ...
  - It listens for service changes in tunnel services according to the string prefix "PhobosTunnel$".
  - It manages DPAPI-encrypted configuration files in `C:\Program Files\Phobos\Data`, which is created with `O:SYG:SYD:PAI(A;OICI;FA;;;SY)(A;OICI;FA;;;BA)`, and makes some effort to enforce good configuration filenames.

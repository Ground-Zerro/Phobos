...
The uninstaller will clean up the entirety of `HKLM\Software\Phobos`. Use
at your own risk, and please make sure you know what you're doing.

#### `HKLM\Software\Phobos\LimitedOperatorUI`
...
```
> reg add HKLM\Software\Phobos /v LimitedOperatorUI /t REG_DWORD /d 1 /f
```

#### `HKLM\Software\Phobos\DangerousScriptExecution`
...
utmost trepidation. Rather than use `%i`, Phobos instead sets the
environment variable `WIREGUARD_TUNNEL_NAME` to the name of the tunnel when
executing these scripts.

```
> reg add HKLM\Software\Phobos /v DangerousScriptExecution /t REG_DWORD /d 1 /f
```

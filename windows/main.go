...
func checkForWow64() {
	...
	if b {
		fatalf("You must use the native version of Phobos on this computer.")
	}
}
...
func checkForAdminGroup() {
	...
	if !elevate.TokenIsElevatedOrElevatable(processToken) {
		fatalf("Phobos may only be used by users who are a member of the Builtin %s group.", elevate.AdminGroupName())
	}
}
...
func checkForAdminDesktop() {
	...
	if !adminDesktop && err == nil {
		fatalf("Phobos is running, but the UI is only accessible from desktops of the Builtin %s group.", elevate.AdminGroupName())
	}
}

func launchElevatedManagerServiceInstaller() error {
...
func execElevatedManagerServiceInstaller() error {
	if err := launchElevatedManagerServiceInstaller(); err != nil {
		return err
	}
	os.Exit(0)
...
func main() {
	...
	case "/installmanagerservice":
		...
		fatalf("Phobos system tray icon did not appear after 30 seconds.")
		return
	...
	case "/importlink":
		...
		link := os.Args[2]
		if !conf.IsPhobosLink(link) {
			fatalf("Not a phobos:// link: %s", link)
		}
		if ui.SendImportLink(link) {
			return
		}
		checkForAdminGroup()
		if err := launchElevatedManagerServiceInstaller(); err != nil {
		...
		ui.WaitForUIThenImportLink(link, time.Minute)
		return
	case "/tunnelservice":
		if len(os.Args) != 3 {
			usage()
	...
	case "/removedriver":
		...
		if err := wintun.Uninstall(); err != nil {
			log.Printf("Error removing the Wintun driver: %v\n", err)
		}
		return
	}
	usage()
